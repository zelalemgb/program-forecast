...
AS $function$
    WITH filtered_data AS (
        SELECT * FROM procurement_analytics
    ...
    SELECT COUNT(*)::BIGINT, COALESCE(SUM(total_cost), 0), COALESCE(AVG(price), 0),
           COUNT(DISTINCT facility_name)::BIGINT, COUNT(DISTINCT product_name)::BIGINT,
           CASE WHEN COUNT(*) > 0 THEN (COUNT(*) FILTER (WHERE procurement_source = 'EPSS') * 100.0 / COUNT(*)) ELSE 0 END
    FROM filtered_data;
$function$;
...
-- 7) Revoke API access to materialized view if it exists (use TABLE privileges for matviews)
DO $$
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'REVOKE ALL PRIVILEGES ON TABLE public.procurement_monthly_summary FROM anon, authenticated';
  END IF;
END$$;