...
-- Handle duplicate zones by adding a suffix to make them unique
UPDATE public.zone 
SET zone_name = zone_name || ' (' || zone_id || ')'
...
-- Handle duplicate woredas by adding a suffix to make them unique  
UPDATE public.woreda 
SET woreda_name = woreda_name || ' (' || woreda_id || ')'
...
-- Add unique constraints for the hierarchy
