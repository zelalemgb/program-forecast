-- Phase 1: Core schema for Program Setup, Requests, Workflow, Finance, Documents, Comments
-- NOTE: Uses existing helper functions: set_owner_user_id, update_updated_at_column, audit_trigger, has_role
...
CREATE POLICY IF NOT EXISTS "Programs are readable by authenticated" ON public.programs
FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Admins/Analysts can modify programs" ON public.programs
...
CREATE POLICY IF NOT EXISTS "Program settings readable by authenticated" ON public.program_settings
FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Admins/Analysts can modify program settings" ON public.program_settings
FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'analyst'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'analyst'::app_role));
...
CREATE POLICY IF NOT EXISTS "Funding sources readable by authenticated" ON public.funding_sources
FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Admins/Analysts can modify funding sources" ON public.funding_sources
FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'analyst'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'analyst'::app_role));
...
CREATE POLICY IF NOT EXISTS "Funding allocations readable by authenticated" ON public.program_funding_allocations
FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY IF NOT EXISTS "Admins/Analysts can modify funding allocations" ON public.program_funding_allocations
FOR ALL TO authenticated USING (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'analyst'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'analyst'::app_role));
...
-- RLS: owners or admins can view; owners can modify drafts; admins can modify all
CREATE POLICY IF NOT EXISTS "Owners/Admins can select procurement_requests" ON public.procurement_requests
FOR SELECT USING ((user_id = auth.uid()) OR has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'analyst'::app_role));

CREATE POLICY IF NOT EXISTS "Owners can insert their requests" ON public.procurement_requests
FOR INSERT WITH CHECK (user_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Owners can update draft or returned" ON public.procurement_requests
FOR UPDATE USING ((user_id = auth.uid()) AND (status IN ('draft','returned')))
WITH CHECK ((user_id = auth.uid()) AND (status IN ('draft','returned')));

CREATE POLICY IF NOT EXISTS "Admins can update any requests" ON public.procurement_requests
...
-- RLS: based on parent request ownership or admin/analyst
CREATE POLICY IF NOT EXISTS "Access items via parent request (select)" ON public.procurement_request_items
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Owners insert items for own request" ON public.procurement_request_items
...
CREATE POLICY IF NOT EXISTS "Owners update items for editable requests" ON public.procurement_request_items
FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
...
CREATE POLICY IF NOT EXISTS "Admins manage items" ON public.procurement_request_items
...
CREATE POLICY IF NOT EXISTS "Read transitions if can read request" ON public.request_transitions
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
...
CREATE POLICY IF NOT EXISTS "Insert transitions if owner/admin/analyst" ON public.request_transitions
FOR INSERT WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
    WHERE pr.id = request_transitions.request_id
...
CREATE POLICY IF NOT EXISTS "Read events if can read request" ON public.procurement_events
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
...
CREATE POLICY IF NOT EXISTS "Insert/Update events by admin/analyst" ON public.procurement_events
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
    WHERE pr.id = procurement_events.request_id
...
CREATE POLICY IF NOT EXISTS "Read finance if can read request" ON public.finance_status
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
...
CREATE POLICY IF NOT EXISTS "Admins can modify finance" ON public.finance_status
FOR ALL USING (has_role(auth.uid(), 'admin'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY IF NOT EXISTS "Read documents if can read request" ON public.request_documents
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
...
CREATE POLICY IF NOT EXISTS "Insert documents if owner/admin/analyst" ON public.request_documents
FOR INSERT WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
    WHERE pr.id = request_documents.request_id
...
CREATE POLICY IF NOT EXISTS "Read comments if can read request" ON public.request_comments
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
...
CREATE POLICY IF NOT EXISTS "Insert comments if owner/admin/analyst" ON public.request_comments
FOR INSERT WITH CHECK (
  EXISTS (
    SELECT 1 FROM public.procurement_requests pr
    WHERE pr.id = request_comments.request_id
...
-- Storage policies for procurement-docs
DO $$ BEGIN
